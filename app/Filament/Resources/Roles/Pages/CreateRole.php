...
class CreateRole extends CreateRecord
{
    ...
    protected function mutateFormDataBeforeCreate(array $data): array
    {
        $this->permissions = collect($data)
            ->filter(fn (mixed $permission, string $key): bool => ! in_array($key, ['name', 'guard_name', 'select_all', Utils::getTenantModelForeignKey(), 'users']))
            ->values()
            ->flatten()
            ->unique();

        // Extract users data before removing it from the data array
        $users = $data['users'] ?? [];
